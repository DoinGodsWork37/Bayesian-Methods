...
```{r Different-sigma}
multi_start <- function(runs, iteration, seed = NULL, sd) {
  set.seed(seed)
  init <- rerun(length(runs), sample(-75:75, 2))
...
```{r Animate, gganimate = list(nframes = 200, fps = 30, device = "svg", duration = 30), eval=FALSE}
runs <- 1:8
iterations <- 25000
sd <- .1032424
dat <- multi_start(runs = runs, iteration = iterations, sd = sd)
...
animate(a)
ggsave("markov_chain_monte_carlo.png", width = 6, height = 4)

rerun(10, dat$plot_data %>%
  group_by(run) %>% 
  mutate(step = as.numeric(row_number()))) %>%
  bind_rows() %>%
  .[complete.cases(.), ] %>% 
...
gganimate::save_animation(gganimate::last_animation(), "markov_chain_monte_carlo.gif")
```
