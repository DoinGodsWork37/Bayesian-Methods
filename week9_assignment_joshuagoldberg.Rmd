...
1. Fit model described on the diagram:
```{r, echo=FALSE}
...
Additionally, the densities overlap nearly completely. Note the densities for `model_no_hyper_parameter` overlap due to the exclusion of the hyper prior. The upshot is all the groups get a the same $\sigma$.
```{r}
plot(
  u_density,
  ylim = c(0, .10),
...
Eliminating the hyper parameter $\sigma\alpha$ resulted in MCMC estimating the actual group differences in the data, as seen in the plot above. This essentially resulted in a fixed effects model, while including the hypter parameter introduces random effects that focus on the population rather than the difference between the groups. 
